...
export function outboundCampaignsRoutes() {
  ...
  // Auto top-up: for every active email_campaign, queue a discovery run when
  // its uncontacted in-band lead pool drops below `threshold`. Idempotent —
  // skips campaigns that already have a recent pending/running run.
  //
  // Designed to be hit on a daily cron (Google Cloud Scheduler → POST here).
  // Body params: { threshold = 50, batch = 200, dryRun = false }
  router.post("/auto-discover", async (req, res) => {
    try {
      const teamId = await getDefaultTeamId();
      const threshold = Math.max(Number(req.body?.threshold) || 50, 0);
      const batch = Math.min(Math.max(Number(req.body?.batch) || 200, 10), 500);
      ...
      for (const c of campaigns || []) {
        ...
        // Count uncontacted in-band leads. Same JS-filter trick as /leads —
        // raw_data->>'estimated_sales' is text in jsonb so DB-side numeric
        // compare is unsafe. Cap at 5000 (way more than the threshold ever).
        ...
        // Skip if a recent run is already in-flight — don't pile up.
      ...
      res.json({ threshold, batch, dryRun, campaigns: summary });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
