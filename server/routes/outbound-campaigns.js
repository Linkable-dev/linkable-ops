...
async function rollupCampaignMetrics(teamId, campaignId) {
  ...
  for (const r of data || []) {
    ...
    const buckets = [m];
    if (r.brand_group) {
      m.byGroup[r.brand_group] = m.byGroup[r.brand_group] || emptyBucket();
      buckets.push(m.byGroup[r.brand_group]);
    }
    if (r.touch_number) {
      const tk = `T${r.touch_number}`;
      m.byTouch[tk] = m.byTouch[tk] || emptyBucket();
      buckets.push(m.byTouch[tk]);
    }
    if (r.brand_group && r.touch_number) {
      const sk = `${r.brand_group}-T${r.touch_number}`;
      m.bySlot[sk] = m.bySlot[sk] || emptyBucket();
      buckets.push(m.bySlot[sk]);
    }
    for (const b of buckets) {
      b.total++;
      if (left_lobby) b.sent++;
      if (r.delivered_at) b.delivered++;
      if (r.bounced_at) b.bounced++;
      if (r.opened_at) b.opened++;
      if (r.clicked_at) b.clicked++;
      if (r.replied_at) b.replied++;
      if (r.complained_at) b.complained++;
    }
  ...
  for (const b of Object.values(m.byGroup)) finalizeRates(b);
  for (const b of Object.values(m.byTouch)) finalizeRates(b);
  for (const b of Object.values(m.bySlot)) finalizeRates(b);
  return m;
}
